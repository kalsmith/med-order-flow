<div>
    ...
    <div class="mb-4 text-start">
        <h6 class="fw-bold mb-3"><i class="bi bi-people me-2 text-primary"></i>¿Para quién es la orden?</h6>
        <div class="row g-3">
            @foreach($patients as $p)
                <div class="col-6 col-md-4" wire:key="p-{{ $p->id }}">
                    <div wire:click="selectPatient({{ $p->id }})"
                         class="patient-card p-3 text-center transition-all h-100 {{ $selected_patient_id == $p->id ? 'active shadow-sm' : '' }}">
                        ...
                        <span class="text-muted d-block" style="font-size: 0.7rem;">{{ $p->relationship == 'self' ? 'Tú' : ucfirst($p->relationship) }}</span>
                    </div>
                </div>
            @endforeach
            <div class="col-6 col-md-4">
                <div wire:click="toggleAddFamily" class="patient-card p-3 text-center h-100 border-dashed {{ $showAddFamily ? 'active' : '' }}" style="border-style: dashed !important;">
    ...
    @if($showAddFamily)
        <div class="card border-0 rounded-4 mb-4 animate__animated animate__fadeIn" style="background-color: #f1f5f9;">
            <div class="card-body p-4 text-start">
                <h6 class="fw-bold mb-3 small text-uppercase text-primary">Datos del Familiar</h6>
                <div class="row g-3">
                    <div class="col-12">
                    ...
                    <div class="col-12 mt-4">
                        <button wire:click="saveFamily" wire:loading.attr="disabled" class="btn btn-primary w-100 fw-bold rounded-pill">
