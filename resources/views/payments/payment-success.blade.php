                ...
                <div class="card card-success border-0 shadow-sm p-4">
                    <div class="card-body text-center p-md-4">
                        ...
                        <div class="bg-light p-4 rounded-4 text-start mb-4 border">
                            <h6 class="fw-bold border-bottom pb-2 mb-3 text-uppercase small text-secondary">Detalle del Comprobante</h6>
                            <dl class="row mb-0 small">
                                <dt class="col-sm-5 text-muted">Nº Orden:</dt>
                                <dd class="col-sm-7 fw-semibold text-dark">{{ $order->id ?? 'N/A' }}</dd>
                                ...
                                <dd class="col-sm-7 text-primary fw-bold fs-6">$ {{ number_format($order->amount ?? 0, 0, ',', '.') }}</dd>

                                <dt class="col-sm-5 text-muted">Medio de Pago:</dt>
                                <dd class="col-sm-7">
                                    {{-- Lógica para mejorar la legibilidad --}}
                                    @php
                                        $method = $order->paymentTransaction?->metadata['payment_method'] ?? 'Flow';
                                        echo ($method === 'unknown') ? 'Flow / Transferencia' : ucfirst($method);
                                    @endphp
                                </dd>

                                <dt class="col-sm-5 text-muted">Paciente:</dt>
                                <dd class="col-sm-7">{{ auth()->user()->name ?? 'Usuario' }}</dd>
                            </dl>
                        </div>
